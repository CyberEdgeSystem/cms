...
@section('content')
        ...
        <div class="bg-[#fcfcfd] border border-gray-200 rounded-lg px-4 py-2 pb-4 w-80 flex flex-col">
            <h1 class="text-2xl text-center">Логін</h1>
            @if ($errors->any())
            ...
            @endif
            <form action="{{ route('login') }}" method="POST" class="space-y-3 flex flex-col">
                @csrf
                <div>
                    <label for="email" class="text-sm text-gray-500">Логін</label>
                ...
                <button type="submit" class="px-3 py-1 pb-1.5 bg-[#0067c0] rounded text-gray-100">Увійти</button>
            </form>
        </div>
@endsection
